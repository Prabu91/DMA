<div>
    <div class="mb-6 flex items-center justify-between gap-3">
        <div>
            <h1 class="text-lg font-medium text-ink">Riwayat booking</h1>
        ...
        @forelse ($this->orders as $order)
            <a href="{{ route('sekolah.riwayat.show', $order->id) }}" wire:navigate
               class="flex items-center justify-between gap-3 border-b border-line px-5 py-3.5 last:border-b-0 hover:bg-page">
                <div class="min-w-0">
                    <div class="flex items-center gap-2">
                        @if ($order->booking_code)
                            <span class="text-sm font-medium text-ink">{{ $order->booking_code }}</span>
                        @else
                            <span class="text-sm text-ink">Booking #{{ $order->id }}</span>
                ...
                <div class="text-sm font-medium text-ink">Rp{{ number_format($order->total, 0, ',', '.') }}</div>
            </a>
        @empty
