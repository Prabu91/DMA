<div class="h-full overflow-y-auto pb-16">
    <div class="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:py-8">
        <div class="mb-6 flex flex-wrap items-end justify-between gap-3">
            <div>
                <h1 class="text-xl font-semibold text-ink">{{ $lihatArsip ? 'Archived boards' : 'Board' }}</h1>
                <p class="text-sm text-ink-muted">Delapan Mata Air workspace</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <button type="button" wire:click="$toggle('lihatArsip')"
                    ...
                    {{ $lihatArsip ? '← Back to active boards' : 'See archived boards' }}
                </button>
                @unless ($lihatArsip)
                    <x-button wire:click="bukaFormBuat">+ Create board</x-button>
                @endunless
            </div>
        </div>
        ...
        @if ($pesan)
            <div class="mb-4 flex items-center justify-between gap-3 rounded-lg border border-line bg-card px-4 py-3 text-sm text-ink" role="status">
                <span>{{ $pesan }}</span>
                <button type="button" wire:click="$set('pesan', null)" class="font-medium text-navy underline">Close</button>
            </div>
        @endif
                                ...
                                @if ($b->saya_kelola)
                                    <div class="absolute right-2 top-2 flex gap-1">
                                        <button type="button" wire:click="pulihkan({{ $b->id }})"
                                                class="rounded-md bg-white/90 px-2 py-1 text-xs font-medium text-ink hover:bg-white">Restore</button>
                                        @unless ($b->isOrder())
                                            <button type="button" wire:click="hapus({{ $b->id }})"
                                                    wire:confirm="Delete board &quot;{{ $b->nama }}&quot; selamanya beserta semua list, kartu, komentar, dan lampirannya? Actions ini tidak bisa dibatalkan."
                                                    class="rounded-md bg-[#C9372C] px-2 py-1 text-xs font-medium text-white hover:bg-[#AE2E24]">Delete</button>
                                        @endunless
                                    </div>
                                @endif
                            @else
                                <button type="button" wire:click="bintang({{ $b->id }})"
                                        aria-label="{{ $b->saya_bintang ? 'Delete bintang' : 'Beri bintang' }} {{ $b->nama }}"
        ...
        @empty
            <p class="rounded-xl border border-line bg-card px-4 py-10 text-center text-sm text-ink-muted">
                {{ $lihatArsip ? 'No archived boards.' : 'No boards yet.' }}
            </p>
        @endforelse
    </div>
    ...
    @if ($bukaBuat)
        <div class="fixed inset-0 z-50 flex items-end justify-center sm:items-center" wire:key="buat-board"
            ...
            <form wire:submit="buat" role="dialog" aria-modal="true" aria-labelledby="judul-buat-board"
                ...
                <h2 id="judul-buat-board" class="text-base font-semibold text-ink">Create board</h2>
                ...
                <fieldset class="mt-4">
                    <legend class="text-sm font-medium text-ink">Background colour</legend>
                ...
                <div class="mt-4 space-y-1.5">
                    <label for="visibilitas-board" class="block text-sm font-medium text-ink">Who can see it</label>
                ...
                <div class="mt-5 flex items-center justify-end gap-2">
                    <x-button type="button" variant="ghost" wire:click="$set('bukaBuat', false)">Cancel</x-button>
                    <x-button type="submit">
                        <span wire:loading.remove wire:target="buat">Create</span>
                        <span wire:loading wire:target="buat">Creating…</span>
                    </x-button>
                </div>
            </form>
        </div>
